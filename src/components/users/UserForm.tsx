...
/**
 * UserForm component - Displays a form for creating/editing users
...
const UserForm = ({ onSubmit, initialData, onCancel, organizations }: Props) => {
  ...
  useEffect(() => {
    if (initialData) {
      const orgId =
        typeof initialData.organizacion === 'object' &&
        initialData.organizacion !== null
  ...
  return (
    ...
    >
      ...
      <div className="mb-3">
        ...
        {errors.organizacion && (
          <p className="text-danger">{errors.organizacion.message}</p>
        )}
      </div>
